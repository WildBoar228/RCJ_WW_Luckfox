...
namespace vision {
    ...
    FrameFetcher::FrameFetcher()
        : width(vision_cfg.frame_width), height(vision_cfg.frame_height) {
        system("RkLunch-stop.sh");

        memset(fps_text,0,16);

        if (width <= 0 || height <= 0) {
            RK_LOGE("invalid frame size: %dx%d", width, height);
            std::exit(-1);
        }
        ...
        if (stFrame.pstPack == nullptr) {
            RK_LOGE("failed to allocate VENC packet");
            std::exit(-1);
        }
        ...
        if (SAMPLE_COMM_ISP_Init(
                0, hdr_mode, multi_sensor, iq_dir) != RK_SUCCESS ||
            SAMPLE_COMM_ISP_Run(0) != RK_SUCCESS) {
            RK_LOGE("ISP initialization failed");
            std::exit(-1);
        }
        ...
        // vi init
        if (vi_dev_init() != RK_SUCCESS ||
            vi_chn_init(0, width, height) != RK_SUCCESS) {
            RK_LOGE("VI initialization failed");
            std::exit(-1);
        }
        ...
        if (vision_cfg.send_stream) {
            ...
            if (venc_init(0, width, height, codec_type) != RK_SUCCESS) {
                RK_LOGE("VENC initialization failed");
                std::exit(-1);
            }

            // RTSP init
            g_rtsplive = create_rtsp_demo(554);
            if (g_rtsplive == nullptr) {
                RK_LOGE("RTSP server initialization failed");
                std::exit(-1);
            }
            ...
            rtsp_set_video(
                g_rtsp_session, RTSP_CODEC_ID_VIDEO_H264, nullptr, 0);
            rtsp_sync_video_ts(
                g_rtsp_session, rtsp_get_reltime(), rtsp_get_ntptime());
        }
    ...
    void FrameFetcher::SendStream() {
        ...
        if (s32Ret != RK_SUCCESS) {
            RK_LOGE("RK_MPI_VENC_GetStream %x", s32Ret);
            return;
        }
        ...
        if (g_rtsplive && g_rtsp_session) {
            ...
            RK_LOGE(
                "RTSP handles are invalid: demo=%p session=%p",
                g_rtsplive, g_rtsp_session);
        }
